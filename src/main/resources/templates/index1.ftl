        ...
        function show1() {
            ...
            $.getJSON("/update/"+keyword+"/"+latestId,function (data,status) {
                for(var i =data.length-1; i >=0 ; i--){
                    ...
                    var word = data[i].text.match(/NASA/i);

                    data[i].text = data[i].text.replace(/NASA/i,"<span style=\"color: red; font-weight: bold\">"+word+"</span>");
                    $("#row1").prepend(newDiv);
                }
                for(var i =data.length-1; i >=0 ; i--){
                    $("div#row1 div#"+data[i].id).html("<div class=\"\" style=\"background-color: lemonchiffon;border-style:solid;border-width: 2px;border-bottom-width: 1px;border-top-width: 1px\" >\n" +
                            ...
                            "\n" +
                            "                    </div>");
                    $("div#row1 div#"+data[i].id).delay((data.length-1-i)*1000).slideDown();
                }
            });
